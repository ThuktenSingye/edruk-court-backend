...
# Case Statistics Service Class

# rubocop:disable Metrics/ClassLength, Metrics/MethodLength, Metrics/AbcSize
class CaseStatisticService
  ...
  def decided_case(court)
    court.cases.where(case_status: %i[dismissed withdrawn settled closed]).distinct.count
  end

  def pending_case(court)
    court.cases.where(case_status: :pending).distinct.count
  end

  def appeal_case(court)
    court.cases.where(is_appeal: true).distinct.count
  end

  def enforced_case(court)
    court.cases.where(is_enforced: true).distinct.count
  end

  def total_case(court)
    court.cases.distinct.count
  end
